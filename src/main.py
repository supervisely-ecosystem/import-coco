...
def import_coco(api: sly.Api, task_id, context, state, app_logger):
    ...
    for dataset in coco_datasets:
        ...
        if coco_converter.check_dataset_for_annotation(dataset):
            ...
            for batch in sly.batched(coco_dataset.images, batch_size=10):
                for coco_image in batch:
                    ...
                    img_size = (h, w)
                    coco_annotations = coco_converter.get_coco_annotations_for_current_image(
                        coco_image, coco_dataset.annotations
                    )
                    ann = coco_converter.create_sly_ann_from_coco_annotation(
                        meta, coco_dataset.categories, coco_annotations, img_size
                    )
                    coco_converter.move_trainvalds_to_sly_dataset(
                        dataset, coco_image, ann
                    )
